...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if current_rmse < best_rmse:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"best_rmse_model-RMSE:{current_rmse:.4f}-MAE:{current_mae:.4f}-MAPE:{current_mape:.2f}%-R:{current_r:.2f}%.pth"))
        ...
        if current_mae < best_mae:
            ...
            any_improvement = True
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"best_mae_model-RMSE:{current_rmse:.4f}-MAE:{current_mae:.4f}-MAPE:{current_mape:.2f}%-R:{current_r:.2f}%.pth"))
        ...
        if current_r > best_r:
            best_r = current_r
            any_improvement = True
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"best_r_model-RMSE:{current_rmse:.4f}-MAE:{current_mae:.4f}-MAPE:{current_mape:.2f}%-R:{current_r:.2f}%.pth"))
